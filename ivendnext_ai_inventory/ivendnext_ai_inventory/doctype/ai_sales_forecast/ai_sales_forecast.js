...
function run_ai_sales_forecast(frm) {
    ...
    frappe.call({
        method: 'ivendnext_ai_inventory.ivendnext_ai_inventory.doctype.ai_sales_forecast.ai_sales_forecast.run_ai_forecast',
        args: {
...
function sync_individual_sales_forecast(frm) {
    ...
    frappe.confirm(
        ...
        function() {
            ...
            frappe.call({
                method: 'ivendnext_ai_inventory.ivendnext_ai_inventory.doctype.ai_sales_forecast.ai_sales_forecast.generate_forecast_for_item',
                args: {
...
function run_background_sales_sync() {
    frappe.call({
        method: 'ivendnext_ai_inventory.ivendnext_ai_inventory.doctype.ai_sales_forecast.ai_sales_forecast.enqueue_sync_ai_sales_forecasts',
        callback: function(r) {
            if (r.message) {
                frappe.show_alert({
                    message: r.message.message,
...
function refresh_current_sales_data(frm) {
    ...
    frappe.call({
        method: 'ivendnext_ai_inventory.ivendnext_ai_inventory.doctype.ai_sales_forecast.ai_sales_forecast.get_recent_sales_data',
        args: {
            item_code: frm.doc.item_code,
            customer: frm.doc.customer,
            company: frm.doc.company
...
function create_sales_order(frm) {
    ...
    frappe.confirm(
        ...
        function() {
            ...
            frappe.call({
                method: 'ivendnext_ai_inventory.ivendnext_ai_inventory.doctype.ai_sales_forecast.ai_sales_forecast.create_sales_order_from_data',
                args: {
                    item_code: frm.doc.item_code,
                    customer: frm.doc.customer,
...
function view_sales_history_dialog(frm) {
    ...
    frappe.call({
        method: 'ivendnext_ai_inventory.ivendnext_ai_inventory.doctype.ai_sales_forecast.ai_sales_forecast.get_sales_history',
        args: {
            item_code: frm.doc.item_code,
            customer: frm.doc.customer,
            company: frm.doc.company
...
function show_bulk_sales_forecast_dialog() {
    let dialog = new frappe.ui.Dialog({
        ...
        primary_action: function() {
            ...
            if (values) {
                frappe.call({
                    method: 'ivendnext_ai_inventory.ivendnext_ai_inventory.doctype.ai_sales_forecast.ai_sales_forecast.bulk_create_sales_forecasts',
                    args: values,
                    callback: function(r) {
                        if (r.message) {
                            frappe.show_alert({
