...
class SystemHealthReport {
    ...
    async load_report() {
        try {
            ...
            const response = await frappe.call({
                method: 'ivendnext_ai_inventory.ivendnext_ai_inventory.report.system_health_report.generate_system_health_report',
                args: {
    ...
    async export_report(format) {
        try {
            ...
            const response = await frappe.call({
                method: 'ivendnext_ai_inventory.ivendnext_ai_inventory.report.system_health_report.export_system_health_report',
                args: {
                    company: this.company,
                    format: format
                }
    ...
    async fix_bounds_calculation_error(issue) {
        ...
        for (const forecast of affected_forecasts) {
            try {
                const response = await frappe.call({
                    method: 'ivendnext_ai_inventory.ivendnext_ai_inventory.doctype.ai_financial_forecast.ai_financial_forecast.fix_bounds_issue',
                    args: {
                        name: forecast.name
                    }
                });
    ...
    async fix_data_quality_issues(issue) {
        ...
        try {
            const response = await frappe.call({
                method: 'ivendnext_ai_inventory.ivendnext_ai_inventory.utils.data_quality.fix_data_quality_issues',
                args: {
                    company: this.company,
    ...
    async fix_integration_issues(issue) {
        ...
        try {
            const response = await frappe.call({
                method: 'ivendnext_ai_inventory.ivendnext_ai_inventory.utils.integration.fix_sync_issues',
                args: {
                    company: this.company
                }
            });
