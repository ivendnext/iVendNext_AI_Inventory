...
def queue_forecast_update(forecast_name, delay=1):
    ...
    try:
        frappe.enqueue(
            'ivendnext_ai_inventory.ivendnext_ai_inventory.doctype.ai_inventory_forecast.ai_inventory_forecast.run_forecast_background',
...
def queue_item_forecast_creation(item_code):
    ...
    try:
        frappe.enqueue(
            'ivendnext_ai_inventory.hooks_handlers.create_forecasts_for_new_item',
            item_code=item_code,
            queue='long',
            timeout=600,
            is_async=True,
...
def queue_warehouse_forecast_creation(warehouse, company):
    ...
    try:
        frappe.enqueue(
            'ivendnext_ai_inventory.hooks_handlers.create_forecasts_for_new_warehouse',
            warehouse=warehouse,
            company=company,
            queue='long',
            timeout=1200,
