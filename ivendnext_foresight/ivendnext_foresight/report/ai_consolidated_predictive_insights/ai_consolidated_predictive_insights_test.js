...
function show_quick_recorder_analysis(report) {
    ...
    frappe.call({
        method: "ivendnext_foresight.forecasting.scheduled_tasks.get_quick_recorder_analysis",
        callback: function(r) {
...
function show_revenue_opportunities(report) {
    ...
    frappe.call({
        method: "ivendnext_foresight.forecasting.scheduled_tasks.get_revenue_opportunities",
        callback: function(r) {
            if (r.message && r.message.status === "success") {
                let data = r.message.data;
                let summary = r.message.summary;
...
function show_risk_assessment(report) {
    ...
    frappe.call({
        method: "ivendnext_foresight.forecasting.scheduled_tasks.get_risk_assessment",
        callback: function(r) {
            if (r.message && r.message.status === "success") {
                let data = r.message.data;
                let summary = r.message.summary;
...
function show_demand_forecasting(report) {
    ...
    frappe.call({
        method: "ivendnext_foresight.forecasting.scheduled_tasks.get_demand_forecasting",
        callback: function(r) {
            if (r.message && r.message.status === "success") {
                let data = r.message.data;
                let summary = r.message.summary;
...
function create_ai_purchase_order(report) {
    ...
    frappe.call({
        method: "ivendnext_foresight.forecasting.scheduled_tasks.create_ai_purchase_orders",
        callback: function(r) {
            if (r.message && r.message.status === "success") {
                let data = r.message.data;
                let summary = r.message.summary;
                            ...
                            options: `
                                ...
                                <script>
                                    ...
                                    function create_single_purchase_order(company, supplier, key) {
                                        ...
                                        frappe.call({
                                            method: "ivendnext_foresight.forecasting.scheduled_tasks.create_purchase_order",
                                            args: {
                                                company: company,
                                                supplier: supplier,
                                                items: po_data.items
