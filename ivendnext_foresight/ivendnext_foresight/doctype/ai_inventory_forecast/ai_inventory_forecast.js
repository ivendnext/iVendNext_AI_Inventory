...
function sync_individual_forecast(frm) {
    ...
    frappe.confirm(
        ...
        function() {
            ...
            frappe.call({
                method: 'ivendnext_foresight.hooks_handlers.trigger_immediate_forecast_update',
                args: {
...
function run_background_sync() {
    frappe.call({
        method: 'ivendnext_foresight.ivendnext_foresight.doctype.ai_inventory_forecast.ai_inventory_forecast.enqueue_sync_ai_forecasts',
        callback: function(r) {
            if (r.message) {
                frappe.show_alert({
                    message: r.message.message,
...
function view_stock_levels_dialog(frm) {
    ...
    // Get stock data
    frappe.call({
        method: 'ivendnext_foresight.ivendnext_foresight.doctype.ai_inventory_forecast.ai_inventory_forecast.get_item_stock_levels',
        args: {
            item_code: frm.doc.item_code,
            company: frm.doc.company
        },
