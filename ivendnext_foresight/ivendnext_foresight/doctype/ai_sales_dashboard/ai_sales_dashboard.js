...
function sync_ai_sales_forecasts_now(frm) {
    frappe.confirm(__('This will run AI sales forecasting for all active items and customers. Continue?'), function() {
        ...
        frappe.call({
            method: 'ivendnext_foresight.ivendnext_foresight.doctype.ai_sales_forecast.ai_sales_forecast.sync_ai_sales_forecasts_now',
            callback: function(r) {
                if (r.message) {
                    frappe.msgprint({
...
function show_sales_sync_status_dialog(frm) {
    frappe.call({
        method: 'ivendnext_foresight.ivendnext_foresight.doctype.ai_sales_forecast.ai_sales_forecast.get_sales_sync_status',
        callback: function(r) {
...
function bulk_create_for_all_customers(frm) {
    frappe.confirm(__('Create sales forecasts for ALL customers and items? This may take several minutes.'), function() {
        ...
        frappe.call({
            method: 'ivendnext_foresight.ivendnext_foresight.doctype.ai_sales_forecast.ai_sales_forecast.create_forecasts_for_all_customers',
            callback: function(r) {
                if (r.message) {
                    frappe.msgprint({
...
function bulk_create_for_recent_customers(frm) {
    frappe.confirm(__('Create forecasts only for customers with recent sales (last 90 days)? Recommended for better accuracy.'), function() {
        ...
        frappe.call({
            method: 'ivendnext_foresight.ivendnext_foresight.doctype.ai_sales_forecast.ai_sales_forecast.create_forecasts_for_recent_customers',
            callback: function(r) {
                if (r.message) {
                    frappe.msgprint({
...
function bulk_create_sales_orders(frm) {
    frappe.confirm(__('This will create Sales Orders for high-confidence forecasts. Continue?'), function() {
        ...
        frappe.call({
            method: 'ivendnext_foresight.ivendnext_foresight.doctype.ai_sales_forecast.ai_sales_forecast.bulk_create_sales_orders',
            callback: function(r) {
                if (r.message) {
                    frappe.msgprint({
...
function bulk_enable_auto_sales_orders(frm) {
    let dialog = new frappe.ui.Dialog({
        ...
        primary_action: function(values) {
            frappe.call({
                method: 'ivendnext_foresight.ivendnext_foresight.doctype.ai_sales_forecast.ai_sales_forecast.bulk_enable_auto_so',
                args: {
...
function show_sales_analytics_summary(frm) {
    frappe.call({
        method: 'ivendnext_foresight.ivendnext_foresight.doctype.ai_sales_forecast.ai_sales_forecast.get_sales_analytics_summary',
        callback: function(r) {
            if (r.message && r.message.status === 'success') {
...
function show_customer_insights(frm) {
    let dialog = new frappe.ui.Dialog({
        ...
        primary_action: function(values) {
            ...
            frappe.call({
                method: 'ivendnext_foresight.ivendnext_foresight.doctype.ai_sales_forecast.ai_sales_forecast.get_customer_insights',
                args: {
...
function fix_missing_sales_forecasts(frm) {
    frappe.confirm(__('This will create AI Sales Forecasts for customers and items that don\'t have them. Continue?'), function() {
        ...
        frappe.call({
            method: 'ivendnext_foresight.ivendnext_foresight.doctype.ai_sales_forecast.ai_sales_forecast.fix_missing_sales_forecasts',
            callback: function(r) {
                if (r.message) {
                    if (r.message.status === 'success') {
                        frappe.msgprint({
...
function check_sales_forecast_coverage_dialog(frm) {
    frappe.call({
        method: 'ivendnext_foresight.ivendnext_foresight.doctype.ai_sales_forecast.ai_sales_forecast.check_sales_forecast_coverage',
        callback: function(r) {
            if (r.message && r.message.status === 'success') {
...
function display_sales_setup_status(frm) {
    frappe.call({
        method: 'ivendnext_foresight.ivendnext_foresight.doctype.ai_sales_forecast.ai_sales_forecast.get_sales_setup_status',
        callback: function(r) {
            if (r.message && r.message.status === 'success') {
...
function create_bulk_purchase_orders_from_ai(frm) {
    let dialog = new frappe.ui.Dialog({
        ...
        primary_action: function(values) {
            ...
            frappe.call({
                method: 'ivendnext_foresight.ivendnext_foresight.doctype.ai_sales_dashboard.ai_sales_dashboard.create_bulk_purchase_orders_from_ai_analytics',
                args: {
                    filters: values
                },
                callback: function(r) {
...
function show_purchase_order_ai_insights(frm) {
    let dialog = new frappe.ui.Dialog({
        ...
        primary_action: function(values) {
            frappe.call({
                method: 'ivendnext_foresight.ivendnext_foresight.doctype.ai_sales_dashboard.ai_sales_dashboard.get_purchase_order_ai_insights',
                args: {
                    filters: values
                },
                callback: function(r) {
...
function show_smart_procurement_dialog(frm) {
    let dialog = new frappe.ui.Dialog({
        ...
        primary_action: function(values) {
            ...
            frappe.call({
                method: 'ivendnext_foresight.ivendnext_foresight.doctype.ai_sales_dashboard.ai_sales_dashboard.create_smart_procurement_plan',
                args: {
