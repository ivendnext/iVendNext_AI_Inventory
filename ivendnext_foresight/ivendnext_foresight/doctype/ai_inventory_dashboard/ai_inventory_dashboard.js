...
function fix_missing_forecasts(frm) {
    frappe.confirm(__('This will create AI Inventory Forecasts for all items that don\'t have them. This may take several minutes. Continue?'), function() {
        ...
        frappe.call({
            method: 'ivendnext_foresight.ivendnext_foresight.doctype.ai_inventory_forecast.ai_inventory_forecast.fix_item_forecast_creation',
            callback: function(r) {
                if (r.message) {
...
function check_forecast_coverage_dialog(frm) {
    frappe.call({
        method: 'ivendnext_foresight.ivendnext_foresight.doctype.ai_inventory_forecast.ai_inventory_forecast.check_forecast_coverage',
        callback: function(r) {
...
function sync_ai_forecasts_now(frm) {
    frappe.confirm(__('This will run AI forecasting for all active items. Continue?'), function() {
        ...
        frappe.call({
            method: 'ivendnext_foresight.ivendnext_foresight.doctype.ai_inventory_forecast.ai_inventory_forecast.sync_ai_forecasts_now',
            callback: function(r) {
                if (r.message) {
                    frappe.msgprint({
...
function show_sync_status_dialog(frm) {
    frappe.call({
        method: 'ivendnext_foresight.ivendnext_foresight.doctype.ai_inventory_forecast.ai_inventory_forecast.get_simple_sync_status',
        callback: function(r) {
            if (r.message && r.message.status === 'success') {
...
function bulk_create_for_all_items(frm) {
    frappe.confirm(__('Create forecasts for ALL stock items? This may take several minutes.'), function() {
        frappe.call({
            method: 'ivendnext_foresight.hooks_handlers.bulk_create_forecasts_for_existing_items',
            callback: function(r) {
                if (r.message) {
                    frappe.msgprint({
...
function bulk_create_for_items_with_stock(frm) {
    frappe.confirm(__('Create forecasts only for items with stock? This is recommended.'), function() {
        frappe.call({
            method: 'ivendnext_foresight.hooks_handlers.auto_create_forecasts_for_items_with_stock',
            callback: function(r) {
                if (r.message) {
                    frappe.msgprint({
...
function bulk_create_purchase_orders(frm) {
    frappe.confirm(__('This will create Purchase Orders for all items with reorder alerts. Continue?'), function() {
        ...
        frappe.call({
            method: 'ivendnext_foresight.ivendnext_foresight.doctype.ai_inventory_forecast.ai_inventory_forecast.bulk_create_purchase_orders',
            callback: function(r) {
                if (r.message) {
                    frappe.msgprint({
...
function bulk_enable_auto_purchase_orders(frm) {
    let dialog = new frappe.ui.Dialog({
        ...
        primary_action: function(values) {
            frappe.call({
                method: 'ivendnext_foresight.ivendnext_foresight.doctype.ai_inventory_forecast.ai_inventory_forecast.bulk_enable_auto_po',
                args: {
...
function run_analysis_for_company(company) {
    ...
    frappe.call({
        method: 'ivendnext_foresight.ml_supplier_analyzer.run_ml_supplier_analysis',
        args: {
            company: company
        },
        callback: function(r) {
...
function show_supplier_analytics_summary(frm) {
    frappe.call({
        method: 'ivendnext_foresight.ml_supplier_analyzer.get_supplier_analytics_summary',
        callback: function(r) {
            if (r.message && r.message.status === 'success') {
...
function display_setup_status(frm) {
    frappe.call({
        method: 'ivendnext_foresight.ivendnext_foresight.doctype.ai_inventory_forecast.ai_inventory_forecast.get_setup_status',
        args: {
